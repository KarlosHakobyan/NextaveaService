import { createThemeToggle } from '../../shared/theme.js';
import { KEYS, writePref } from '../../shared/prefs.js';
...
export function renderHeader() {
    ...
    header.innerHTML = `
        <div class="header-container">
            <a href="#/" class="logo">Nextavéa <span>Service</span></a>
            <nav>
                <a href="#/">${t.home}</a>
    ...
    if (langSelect) {
        langSelect.addEventListener('change', (e) => {
            // Written to both keys while the old one is still read elsewhere.
            writePref(KEYS.LANG, e.target.value);
            localStorage.setItem('app_lang', e.target.value);
            window.location.reload();
        });

        langSelect.insertAdjacentElement('afterend', createThemeToggle({
...
export function renderFooter() {
    ...
    footer.innerHTML = `
        <div class="footer-content">
            <p>&copy; ${new Date().getFullYear()} Nextavéa Service. (C) Karlos Hakobyan. ${rightsText}</p>
        </div>
    `;
    return footer;
}
...
export function renderServiceCard(service) {
    ...
    card.innerHTML = `
        <div class="card-header">
            <span class="service-icon" data-icon="${service.category}"></span>
